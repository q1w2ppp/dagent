...
async function sendMsg(oid,text){const token=await getToken();const card={config:{wide_screen_mode:true},header:{title:{tag:'plain_text',content:'Design Agent'},template:'purple'},elements:[{tag:'div',text:{tag:'lark_md',content:text}}]};const b=JSON.stringify({receive_id:oid,msg_type:'interactive',content:JSON.stringify(card)});return new Promise((resolve,reject)=>{const r=https.request({hostname:'open.feishu.cn',path:'/open-apis/im/v1/messages?receive_id_type=open_id',method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token,'Content-Length':Buffer.byteLength(b)}},res=>{let d='';res.on('data',c=>d+=c);res.on('end',()=>resolve())});r.on('error',reject);r.write(b);r.end()})}
  ...
  if(req.method==='POST'&&req.url==='/feishu'){
    let body='';req.on('data',c=>body+=c);req.on('end',async()=>{
      try{const d=JSON.parse(body);if(d.type==='url_verification'||d.challenge){res.writeHead(200,{'Content-Type':'application/json'});return res.end(JSON.stringify({challenge:d.challenge||d.token}))}
        ...
        const oid=(ev.sender||{}).sender_id||(d.sender||{}).open_id||'';last={et,text:!!text,img:!!imgKey};
